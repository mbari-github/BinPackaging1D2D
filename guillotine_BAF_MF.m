% Packing sempre offline
close
clc
clear

%% Col1= coord x basso sx/ Col2= coord y basso sx/ Col3= width/ Col 4=height  
% widr=[200,400 ,100,60,70,200,300]';
% heir=[300,300,60,100,70,150,300]';
widr=[200,50]';
heir=[200,300]';
        ...
        if F(i,j) ~=0
            ...
            if  ~isempty(m(1)) && ~isnan(m(1)) && (rects(I(1),3)<=F(i,j-2) && rects(I(1),4)<=F(i,j-1))
                ...
                if rects(I(1),4)<F(i,j-1)
                    if flag
                     F(i+1,c+6:c+11)=[F(i,j-4),F(i,j-3)+rects(I(1),4),new_width2,new_height2,new_area2,1];
                    else
...
rectangle('Position',[packed(i,1),packed(i,2),packed(i,3),packed(i,4)],'FaceColor',rand(1,3));
end
axis equal